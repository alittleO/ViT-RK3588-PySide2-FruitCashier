...
class InferenceProcess(Process):
    def __init__(self, rknn_lite, result_queue):
        super().__init__()
    ...
    def run(self):
        while self.running:
            if not self.image_queue.empty():
                ...
                if image is None:  # 使用None作为停止信号
                    self.running = False
                else:
                    results = run_rknn(self.rknn_lite, image)
    ...
    def stop(self):
        self.process_image(None)  # 发送停止信号
...
class MainWindow(QWidget): 
    def __init__(self, serial_reader, rknn_lite):  
        super().__init__() 
        #串口进程
        ...
        self.inference_process = InferenceProcess(rknn_lite, self.result_queue)
...
if __name__ == "__main__":
    ...
    window = MainWindow(serial_reader, rknn_lite)  
    window.show() 
    sys.exit(app.exec_()) 
