            ...
            while self.running:
                if not self.image_queue.empty():
                    ...
                    if image is None:
                        ...
                        # 筛选高概率结果
                        high_prob_results = [(str(i), prob) for i, prob in enumerate(probabilities[0]) if prob > 0.4]
...
class AddItemDialog(QDialog):
    ...
    def update_display_info(self):
        ...
        current_weight = round(self.parent.get_weight(), 3)
        total_price = round(price * current_weight, 2)
        self.info_label.setText(f"单价：{price} 元\n重量：{current_weight} 公斤\n总价：{total_price} 元")
        
class PaymentDialog(QDialog):
    def __init__(self, parent, total_amount):
        super().__init__(parent)
        self.setWindowTitle("支付选择")
        self.setFixedSize(300, 200)
        
        layout = QVBoxLayout(self)
        
        total_label = QLabel(f"总金额: {total_amount:.2f}元", self)
        total_label.setFont(QFont('MiSans', 18))
        total_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(total_label)
        
        cash_button = QPushButton("现金支付", self)
        cash_button.setFont(QFont('MiSans', 18))
        cash_button.clicked.connect(self.accept)
        layout.addWidget(cash_button)
        
        scan_button = QPushButton("扫码支付", self)
        scan_button.setFont(QFont('MiSans', 18))
        scan_button.clicked.connect(self.accept)
...
class MainWindow(QWidget): 
    def __init__(self, serial_reader, inference_process, result_queue):  
        ...
        ''''''
        self.add_card_button = QPushButton("关闭系统") 
        ...
        self.btn_add.clicked.connect(self.show_add_item_dialog)
    ...
    def show_add_item_dialog(self):
        ...
        if dialog.exec_():
            ...
            self.add_data(self.table, [id2label[selected_id], str(id2price[selected_id]), str(round(self.weight,3)), 
                                   str(round(id2price[selected_id]*self.weight,2))])
    ...
    def settle_accounts(self, table):
        ...
        dialog = PaymentDialog(self, total)
        if dialog.exec_():
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'serial_reader'):
            ...
            if hasattr(self.serial_reader, 'join'):
